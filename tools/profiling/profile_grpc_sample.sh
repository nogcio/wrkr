...
sample_out="$root_dir/tmp/grpc_sample_${duration_seconds}s.txt"
rm -f "$sample_out"

# Run wrkr in background so we can sample its stacks.
"$root_dir/target/profiling/wrkr" run tools/perf/wrkr_grpc_plaintext.lua \
...
# This is a lightweight alternative when Instruments CLI (xctrace) is unavailable.
set +e
sample "$wrkr_pid" "$duration_seconds" -file "$sample_out" >/dev/null 2>&1
sample_status=$?
set -e
...
if [[ $sample_status -ne 0 ]]; then
  ...
  echo "Try running: sample $wrkr_pid $duration_seconds -file $sample_out" >&2
  exit 2
fi
