...
fmt: fmt-rust ## Format all code (Rust)

fmt-rust: ## Format Rust code (cargo fmt)
	$(CARGO) fmt --all

fmt-check: fmt-check-rust ## Verify formatting (Rust)

fmt-check-rust: ## Verify Rust formatting (cargo fmt -- --check)
	$(CARGO) fmt --all -- --check

lint: lint-rust ## Lint all code (Rust)
...
check: check-rust ## Run checks (Rust)
...
perf-gate: ## Run perf regression gate (xtask)
